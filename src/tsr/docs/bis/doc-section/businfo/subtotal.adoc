...
Each Transport Protocol needs to be reported in a separate `Subtotal` element.
...
The value of the `Key` element MUST be the Document Type Identifier value
  of the exchanged Dataset.
  
Each combination of Service Provider ID and Dataset Type needs to be
...
Each combination of Service Provider ID. Dataset Type, Sender Country 
  and Receiver Country needs to be reported in a separate `Subtotal` element.
