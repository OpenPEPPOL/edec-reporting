...
====
IMPORTANT: This subtotal can only be counted by receiving Service Providers, because only those have the full data available.
The outgoing count in this subtotal MUST always be `0`.
====


NOTE: For incoming messages, only the sending Service Provider needs
  to be counted. The other side must always be the reporting Service Provider
  that is already part of the Report header data.
