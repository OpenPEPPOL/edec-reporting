...
<schema xmlns="http://purl.oclc.org/dsdl/schematron"
  ...
  <ns prefix="eur" uri="urn:fdc:peppol:end-user-statistics-report:1.0"/>

  <pattern id="default">
    <let name="cl_spidtype" value="' CertSubjectCN '"/>

    <rule context="/eur:EndUserStatisticsReport">
      <let name="total" value="eur:Total/eur:SendingEndUsers + eur:Total/eur:ReceivingEndUsers"/>
      <let name="empty" value="$total = 0"/>

      <assert id="SCH-EUSR-01" flag="fatal" test="normalize-space(eur:CustomizationID) = 'urn:fdc:peppol.eu:oo:trns:end-user-statistics-report:1.0'"
      >[SCH-EUSR-01] The customization ID MUST use the value 'urn:fdc:peppol.eu:oo:trns:end-user-statistics-report:1.0'</assert>
      <assert id="SCH-EUSR-02" flag="fatal" test="normalize-space(eur:ProfileID) = 'urn:fdc:peppol.eu:oo:bis:reporting:1.0'"
      >[SCH-EUSR-02] The profile ID MUST use the value 'urn:fdc:peppol.eu:oo:bis:reporting:1.0'</assert>

      <assert id="SCH-EUSR-03" flag="fatal" test="$empty or eur:Subtotal/eur:SendingEndUsers[not(. &lt; ../../eur:Subtotal/eur:SendingEndUsers)][1] &lt;= eur:Total/eur:SendingEndUsers"
      >[SCH-EUSR-03] The maximum of all subtotals of SendingEndUsers MUST be lower or equal to Totals/SendingEndUsers</assert>
      <assert id="SCH-EUSR-04" flag="fatal" test="$empty or eur:Subtotal/eur:ReceivingEndUsers[not(. &lt; ../../eur:Subtotal/eur:ReceivingEndUsers)][1] &lt;= eur:Total/eur:ReceivingEndUsers"
      ...
      <assert id="SCH-EUSR-15" flag="fatal" test="$empty or eur:Subtotal[normalize-space(@type) = 'PerDT-PR']"
      >[SCH-EUSR-15] The subtotals per 'Dataset Type ID and Process ID' MUST exist</assert>
        
      <!-- Global uniqueness check per Key -->
      <assert id="SCH-EUSR-13" flag="fatal" test="every $st in (eur:Subtotal[normalize-space(@type) = 'PerDT-PR']),
      ...
      <assert id="SCH-EUSR-14" flag="fatal" test="count(eur:Subtotal[normalize-space(@type) !='PerDT-PR']) = 0"
      >[SCH-EUSR-14] Only allowed subtotal types MUST be used.</assert>
    </rule>

    <rule context="/eur:EndUserStatisticsReport/eur:Header">
      <assert id="SCH-EUSR-05" flag="fatal" test="matches(normalize-space(eur:ReportPeriod), '^[0-9]{4}\-[0-9]{2}$')"
      >[SCH-EUSR-05] The reporting period (<value-of select="normalize-space(eur:ReportPeriod)"/>) MUST NOT contain timezone information</assert>
    </rule>

    <rule context="/eur:EndUserStatisticsReport/eur:Header/eur:ReporterID">
    ...
    <rule context="/eur:EndUserStatisticsReport/eur:Subtotal[normalize-space(@type) = 'PerDT-PR']">
      <let name="name" value="'The subtotal per Dataset Type ID and Process ID'"/>
      <assert id="SCH-EUSR-09" flag="fatal" test="count(eur:Key) = 2"
      >[SCH-EUSR-09] $name MUST have two Key elements</assert>
      <assert id="SCH-EUSR-10" flag="fatal" test="count(eur:Key[normalize-space(@metaSchemeID) = 'DT']) = 1"
      >[SCH-EUSR-10] $name MUST have one Key element with the meta scheme ID 'DT'</assert>
      <assert id="SCH-EUSR-11" flag="fatal" test="count(eur:Key[normalize-space(@metaSchemeID) = 'PR']) = 1"
