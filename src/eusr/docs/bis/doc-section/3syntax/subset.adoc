...
An End User Statistics Report contains subtotals per distinct pairs of Dataset Type and Process ID.
...
in the <<codelist-subsettype>> code list

== Subtotal per Dataset Type and Process ID

This subtotal is the only type that is currently supported by End User Statistics Reporting BIS.
...
must match the total number of sending end users.

The sum of the number of all end users who have received datasets per Dataset Type and Process ID
must match the total number of receiving end users.
